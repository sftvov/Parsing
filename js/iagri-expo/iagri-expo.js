...
// НАСТРОЙКИ
const BASE_URL = 'https://catalog.iagri-expo.com/expositions/exposition/6252';
const OUTPUT_FILENAME = 'iagri-expo-companies.csv';
...
async function parseExhibition() {
    try {
        console.log(`🚀 Начинаем парсинг выставки iAGRI...`);
    ...
    try {
        ...
        for (const element of companyElements) {
            try {
                ...
                // Получаем название компании из блока .comp_name внутри ссылки
                ...
                const fullUrl = companyUrl.startsWith('http') 
                    ? companyUrl 
                    : `https://catalog.iagri-expo.com${companyUrl.startsWith('/') ? companyUrl : '/' + companyUrl}`;

                // Проверяем, что это действительно страница компании (а не другой элемент)
