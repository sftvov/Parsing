...
// НАСТРОЙКИ
const BASE_URL = 'https://upakexpo-online.ru/expositions/exposition/137-upakexpo-2026';
const OUTPUT_FILENAME = 'upakexpo.csv';
const COMPANIES_PER_PAGE = 48; // Типичный шаг пагинации, можно настроить
...
async function parseExhibition() {
    try {
        console.log(`🚀 Начинаем парсинг выставки UPAK EXPO 2026...`);
        ...
        if (allCompanies.length === 0) {
            console.log('❌ Компании не найдены. Проверьте селекторы #scroll_list .scroll_item a и структуру страницы.');
            return;
        }
        ...
        for (let i = 0; i < allCompanies.length; i++) {
            ...
            try {
                ...
                await delay(1000 + Math.random() * 500); // Вежливая задержка
            } catch (error) {
...
async function getAllCompanies() {
    ...
    while (hasMorePages && pageNumber <= 20) { // Защита от бесконечного цикла
        try {
            ...
            for (const element of companyElements) {
                try {
                    ...
                    const companyName = element.getAttribute('title'); // Берём название из атрибута title

                    if (companyUrl && companyName) {
                        const fullUrl = companyUrl.startsWith('http')
                            ? companyUrl
                            : `https://upakexpo-online.ru${companyUrl.startsWith('/') ? companyUrl : '/' + companyUrl}`;
            ...
            await delay(1500); // Задержка между страницами
...
async function parseCompanyContacts(companyUrl) {
    try {
        ...
        if (contactsBlock) {
            ...
            // Ищем email по метке "Email:" или "E-mail:"
            ...
            // Альтернативный поиск по всему тексту блока, если не нашли по меткам
            if (!email) {
            ...
            // Если блок не найден, ищем по всей странице
